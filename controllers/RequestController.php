<?php
require 'models/Request.php';
require 'models/Type_request.php';
/**
 * 
 */
class RequestController
{
    private $model;
    private $Type_request;


    public function __construct()
    {
        try{
            $this->model = new Request;
            $this->type_request = new Type_request;
 
        }catch(PDOException $e){
            die($e->getMessage());
        }
    }
    public function index(){
        require 'views/layout.php';
        $requests = $this->model->getAll();
        $type_requests =$this->type_request->getAll();
        require 'views/requests/list.php';
    }
    public function new()
    {
        require 'views/layout.php';
        $requests = $this->model->getAll();
        require 'views/requests/new.php';
    }
    public function save()
    {
       // $validate_request = $this->model->validateGame($_POST);

        //if($validateGame === true) {
         $query =  $this->model->newRequest($_REQUEST);
          // header('Location: ?controller=request');
          if ($query == 1) {
              require 'views/layout.php';
              require 'views/requests/list.php';
              echo " <script>
              Swal.fire({
                    position:'center',
                    icon:'success',
                    title:'Se ha creado el requerimiento correctamente',
                    showConfirmButton: false,
                    timer:1500
                });
               tiempo('request');

              
              </script>";
          };
        //} else {
           /* $error = ['errorMessage' => $validateGame, 'name' => $_POST['name']];
            require 'views/layout.php';
            require 'views/request/list.php';*/
       //}


    }
    public function edit()
    {
        if(isset($_REQUEST['id'])){
            $id=$_REQUEST['id'];

            $data=$this->model->getById($id);
            $users=$this->users->getAll();
            require 'views/layout.php';
            require 'views/game/edit.php';
        }else{
            echo "Error, no se realizo.";
        }
    }
    public function update()
    {
        if(isset($_POST)){
            $this->model->editGame($_POST);
            header('Location: ?controller=game');
        }else{
            echo "Error, no se realizo";
        }
    }

    public function delete()
    {
        $this->model->deletegame($_REQUEST);
        header('Location: ?controller=game');
    }

    public function updateStatus()
    {
        $movie = $this->model->getById($_REQUEST['id']);
        $data = [];

        if($movie[0]->status_id == 1) { 
            $data = [
                'id' => $movie[0]->id,
                'status_id' => 2
            ];
        } elseif($movie[0]->status_id == 2) {
            $data = [
                'id' => $movie[0]->id,
                'status_id' => 1
            ];
        }

        $this->model->editStatus($data);
        header('Location: ?controller=game');

    }
    public function getById()
    {
        require 'views/layout.php';
        $users = $this->users->getAll();
        require 'views/games/view.php';
    }
   

}
